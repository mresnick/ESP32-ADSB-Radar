...
- `resolve_label()` (`app_main.c`) picks what `radar_target_t.label`
  ...
  `RADAR_LABEL_CALLSIGN` (default, `aircraft_t.flight` - itself already
  guaranteed non-empty, falling back to ICAO hex - see `aircraft_model.c`)
  or `RADAR_LABEL_NONE` (no label at all). `radar_view.c` stays entirely
  unaware of `label_mode` itself - it just skips drawing a label at all
  when `label` is empty, and otherwise draws whatever string is in it.
  Aircraft type/registration were considered as further label options and
  deferred - see TASKS.md item 7 for why (SkyAware's live feed has no such
  fields at all, and the local database it does have them in is too
  expensive to look up inline within a display refresh cycle).
