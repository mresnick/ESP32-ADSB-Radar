...
{
    UWORD i, j;
    // NOTE (ESP32 port): the original RaspberryPi source allocated a full
    // WIDTH*HEIGHT UWORD array on the stack here (115200 bytes) - harmless
    // on a Linux thread's multi-MB stack, but it blows past a FreeRTOS task's
    // stack (a few KB) on ESP32, corrupting memory. Since every pixel is the
    // same color, one row-sized buffer repeated HEIGHT times is equivalent
    // and only needs LCD_1IN28_WIDTH*2 (480) bytes.
    UWORD RowBuffer[LCD_1IN28_WIDTH];
    ...
    for(j = 0; j < LCD_1IN28_HEIGHT; j++){
        DEV_SPI_Write_nByte((uint8_t *)RowBuffer, LCD_1IN28_WIDTH*2);
    }
}
